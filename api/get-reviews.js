...
function getExternalId(shopifyProductId) {
...
export default async function handler(req, res) {
    ...
    const shopifyProductId = req.query.shopifyProductId;
    ...
    try {
        ...
        // Normalize a bit
        const normalized = (reviewsJson?.reviews ?? []).map((r) => {
            const rawName = r.reviewer_name || r.reviewer?.name || r.name || "Verified Buyer";
            // Format as "First L."
